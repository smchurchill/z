...
\begin{algorithm}[h]
	...
	\Begin{
		\If{$n=1$}{
			$P^1=\I$, the standard 1--simplex\;	
		}
		$\sigma_0^n,\sigma_1^n\longleftarrow$ standard $n$--simplices\;
		$P_0^{n-1},P_1^{n-1},\dots,P_n^{n-1}\longleftarrow$ result of Algorithm \ref{alg:nprism} with input $n-1$\;
		\ForEach{$(n-1)$ prism $P_i^{n-1}$}{
			attach $\sigma_0^{n-1}$ of $P_i^{n-1}$ to $F^i(\sigma_0^n)$	of $\sigma_0^n$\;
			attach $\sigma_1^{n-1}$ of $P_i^{n-1}$ to $F^i(\sigma_1^n)$	of $\sigma_1^n$\;
		}
		\ForEach{$n-2$ simplex of $$}{}
