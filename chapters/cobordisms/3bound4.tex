...
\begin{proof}
	...
	Around each vertex $v$ of $X$ we fit an octogon $\nbhd{v}$ such that the edges of the octogon alternate between transverse intersection and nonintersection with an arc of codimension 1 critical values.
	
	Consider $\BB_*=\overline{\BB\setminus\cup_v \nbhd{v}}$.
	The topological boundary of $\BB_*$ consists of the octogonal boundaries of the $\nbhd{v}$.
	...
	Each connected component is either a subset of $R_\infty$ or an open octogonal subset of a region $\R_i$.
	We take the topologinal closure of these octogons in $\BB$ and call these spaces by $D_i$.
...
\begin{proof}
	First consider the vertices in the graph $Y$ obtained in Lemma \ref{lem:bbdecomp}.
	Some of these vertices are outside of the image of $f$, so we disregard them entirely.
	The rest are regular points of $f$, so their preimages are disjoint unions of circles in $M$.
	For a vertex $w$ of $Y$, we call the preimage by $M_w$.
	
	Now, the edges of $Y$.
	Again, some edges of $Y$ are outside of $f(M)$, so we disregard them.
	Of the rest, we first consider those that do not intersect $X$.
	These are strands of regular points of $f$, hence 
