...
\begin{proof}%[Proof of Theorem \ref{thm:3bound4}]
	...
	It is shown in \cite{CostThur08} that filling in $f\inv(\pd\,\varepsilon(x))$ with 2--discs and gluing the two genus 3 handlebodies together over $f\inv(\pd\,\varepsilon(x))$ results in a Heegard splitting of $S^3$.
	...
	Let $\varepsilon(\alpha)$ be an arc sleeve, and consider the connected components of $f\inv(\varepsilon(\alpha))$.
	...
	Finally, let $x$ be a codimension 2 critical value and let $\varepsilon(x)$ be its sleeve.
