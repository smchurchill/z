...
Throughout this section $M$ is a closed orientable 3--manifold and $f$ is a generic smooth map $M\to\BB$, so we establish some facts about these maps before proceeding.
...
Let $X_f$ be the image of $S(f)$ through $f$ and $Y_f$ the image of $M$ through $f$.
...
\begin{theorem}[Theorem 3.15 of \cite{Saeki} restricted to orientable 3--manifolds]
	\label{thm:saekising}
...
\begin{proof}
	...
	Next, let $D_{1,k}\in D_1$.
	The preimage $f\inv(D_{1,k})$ has structure determined by whether the arc $\alpha$ of codimension 1 critical points that bisects $D_{1,k}$ into the discs $D_{1,k}^+$ and $D_{1,k}^-$ is the image of a definite or indefinite fold.
%	A disc $D_{1,k}$ in $D_1$ is bisected into  by an arc $\alpha$ of codimension 1 critical points, and that arc is the image of either a definite fold or an indefinite fold.
	First, we focus on the connected components of $D_{1,k}$ that contain no critical points of $f$
%	Each of $D_{1,k}^\pm$ consists entirely of regular values of $f$, so $f\inv(D_{1,k}^\pm)$ is shown to be a disjoint collection of cornered $(3,1)$--handlebodies of genus 1.
	
	For $q\in\alpha$, consider $f\inv(q)$.
	Each connected component of $f\inv(D_{1,k})$ that contains a regular fiber over $q$ is a cornered 3--manifold homeomorphic to a $(3,1)$--handlebody of genus 1 by the same argument we made for $D_0$ above.
	
	Now suppose a component $K$ of $f\inv(D_{1,k})$ contains a singular fiber over $q$.
	Call the critical point in $K$ by $p$.
	If $p$ is a definite fold, then we have local coordinates $(u,z_1,z_2)$ in a neighbourhood $U$ about $p$ and $(x,y)$ about $q$ so that $f$ has the form $(x,y)=(u,\pm(z_1^2+z_2^2))$ on $U$.
	In other words, $f\inv(D_{1,k})\cap U$ has the structure of a disc crossed with an interval.
	
	form of a disc (in $z_1$, $z_2$ coordinates) crossed with an interval (in the $u$ coordinate)
	
	Then we use the orientability of $M$ along with our guaranteed local coordinates about $p$ and $q$ to 
%	 Theorem~\ref{thm:saekising} 
	...
	Then a connected component of $f\inv(q)$ is either a circle or contains a critical point of $f$.
	By design, exactly one connected component of $f\inv(q)$ contains a critical point of $f$.
	For the rest, each connected component of $D_{1,k}$ that does not contain a critical point of $f$ is a cornered 3--manifold homeomorphic to a $(3,1)$--handlebodies of genus 1 by the same argument we made for $D_0$ above.
	
	Next, let $s_q$ be the fiber of $f$ above $q$ that contains a critical point of $f$.
	
	
	
	
	is the disjoint union of a collection of circles along with a connected component that contains the critical point $p$ of $f$ that maps over $q$
		...
		In either case, let $\gamma$ 
	
	
	the case of a definite fold, 
	
	In both cases let $p\in S(f)$ with $f(p)=q\in\alpha$ and consider the local coordinates $(u,z_1,z_2)$ on $U$ about $p$ such that $f$ takes the form of either $(x,y)=(u,\pm(z_1^2+z_2^2))$ (if $p$ is a definite fold point), or $(x,y)=(u,\pm(z_1^2-z_2^2))$ (if $p$ is an indefinite fold point) on $f(U)$ about $f(p)$.
	
	A simple arc in $f(U)$ transverse to $\alpha$
	
	
	The orientability of $M$ guarantees that
...
%	We first consider the case where a component $D_{1,k}$ lies on the boundary of $f(M)$, i.e.\ when $D_{1,k}\cap A\neq\emptyset$.
	The disc $D_{1,k}$ is bisected by an arc $\alpha$ of codimension 1 critical points into a pair of cornered discs.
%	One of these discs is outside of $f(M)$.
	Call these discs $D_{1,k}^1$ and $D_{1,k}^2$.
	Then $D_{1,k}$ has faces $F$, $G$, $FG_1$ and $FG_2$ as in Lemma~\ref{lem:bbdecomp}, labelled so that $D_{1,k}^1$ has $FG_1$ as a face and $D_{1,k}^2$ has $FG_2$ as a face.
	...
	Let $\gamma(t)$ be a simple curve with one endpoint in $FG_1$ and the other in $FG_2$ so that $\gamma$ intersects alpha transversely at exactly one point.
	Then $\gamma\inv\comp f$ is a Morse function $M\to\R$
