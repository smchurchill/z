...
The following lemmas build the framework for our main theorem, the statement of which is that given a closed orientable 3--manifold $M$ and a generic smooth map $f:M\to\BB$, there exists a cobordism $(M,\emptyset)$ induced by $f$.
...
%	then the singular fiber above any point in $\alpha$ is of the second form in Figure~\ref{fig:saekising}.
	local coordinates $(u,z_1,z_2)$ tell us that $f$ has the form of $f(u,z_1,z_2)=(u,\pm(z_1^2-z_2^2))$ near the fold, where the fold points correspond to the line $z_1=z_2=0$ in these local coordinates.
...
%%	We write the decomposition in two ways.
%%	The first way is as
%%	\[
%%		M^+ = H_0\cup H_1\cup H_2\cup H_3
%%	\]
%%	where an element of $H_i$ is a 3--manifolds with faces that is homeomorphic to a $(3,1)$--handlebody of genus i.
%%	The second is as
%	We write the decomposition as
%	\[
%		M_0^+ = f\inv D_0\cup f\inv D_1\cup f\inv D_2
...
%
%%	In other words, call the resulting 4--manifold adjunction $W_1$.
%%	Then the boundary components of $W_1$ are $M_1^-$, identical to $M_0^1$, and $M_1^+$, which has a decomposition into collection of copies of $S^2\times\D^1$, one per connected component of $f\inv D_1$, plus 
%\end{lem}
...
%%	that are homeomorphic to $(3,2)$--handlebodies.
%	%	In other words, call the resulting 4--manifold adjunction $W_1$.
%	%	Then the boundary components of $W_1$ are $M_1^-$, identical to $M_0^1$, and $M_1^+$, which has a decomposition into collection of copies of $S^2\times\D^1$, one per connected component of $f\inv D_1$, plus 
%\end{lem}
