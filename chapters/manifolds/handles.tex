...
\begin{defn}
  ...
  Less formally, this corresponds to the number of independant directions in $M$ along which $f$ decreases.
...
\begin{definition}
  Let $f:M\to\R$ be a Morse function on a closed $n$--manifold $M$ with critical points $\{p_0,\dots,p_k\}$ of indices $\{\lambda_0,\dots,\lambda_k\}$, such that $f(p_i)<f(p_{i+1})$ and $\lambda_i\leq\lambda_{i+1}$ for $i\geq 0$.
  The previous theorems guarantee that $M$ is diffeomorphic to 
\end{definition}
