...
\begin{defn}	
	...
	Let $M=U\cup_f V$ be a Heegaard splitting of $M$, and $\alpha$, $\beta$ a pair of simple, closed, essential curves in $H$.
	If $\alpha$ is a meridian of $U$, $\beta$ is a meridian of $V$, and $|\alpha\cap\beta|=1$, then the pair $(\alpha,\beta)$ is a \emph{stabilization} of the splitting.
\end{defn}

\begin{theorem}
	Let $M$ be a closed, orientable 3--manifold.
	If $M$ has a Heegaard splitting of genus 0, then $M$ is homeomorphic to $S^3$.
\end{theorem}

\begin{theorem}
	Let $M$ be a closed, orientable 3--manifold with a Heegaard splitting $U\cup_f V$ of genus $g$.
	If there is a collection of $g$ disjoint stabilizations for the Heegaard splitting of $M$, then $M$ is homeomorphic to $S^3$.
\end{theorem}
...
\begin{theorem}
	Every closed, orientable 3--manifold can be realized as a Heegaard splitting.
\end{theorem}

Our handle decompositions usually arise from Morse functions as discussed in Chapter \ref{cha:cobordisms}.
In general, attaching maps may be isotoped in order to inject more structure into a handle decomposition.
...
\begin{defn}
	A space $V$ that is homeomorphic to $S^1\times\DD$ is called a \emph{solid torus}, and is a handlebody of genus 1.
	A simple closed curve $J$ in $\pd V$ that intersects a meridian at a single point is called a \emph{longitude}.
	Note that a longitude is essential in $V$, and that there are infinitely many isotopy classes of longitudes of $V$, whereas there's exactly one isotopy class of meridians.
\end{defn}
