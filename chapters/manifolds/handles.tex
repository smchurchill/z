...
The handle decompositions from $f$ and $-f$ are dual in the sense that every $\lambda$--handle attached in the making of the decomposition from $f$ has a corresponding $\mu$--handle in the decomposition from $-f$.
To see this, we will build a handle decomposition
\[
  \emptyset = M_{k+1}^* \subset M_k^* \subset M_{k-1}^* \subset \cdots \subset M_1^* \subset M_0^* = M.
\]
Starting with $M_{k+1}^*$

let's start with $M_k$.
Because $M$ is closed, the handle attachment corresponding to $M_{k-1}\subset M_k$ is of index $n$,
