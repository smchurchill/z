...
\begin{defn}[Bundle]
	...
	\]
	such that the map $v\mapsto h(b,v)$ is a vector space isomorphism $\RRN\mapsto V_b$ for every $b\in U$.
	Such a homeomorphism is called a \emph{local trivialization}, and such a pair is a \emph{local coordinate system}.
	If it is possible to take $U$ to be all of $B$, then $\beta$ is a \emph{trivial bundle}.
	By using a local trivialization, any point $p\in E$ may be represented by a pair $p=(b,v)$ with $b\in B$, $v\in \RRN$, and $\pi(b,v)=b$.
...
Similar to the tubular neighbourhood is the \emph{closed tubular neighbourhood}, which is defined by restricting the vector space fibres $N_M X$ to the closed $n$--disc.
...
\begin{defn}
	...
	Let $Z$ be a manifold with boundary and let $X$ be a submanifold of $M=\pd Z$.
	The normal bundle of $X$ in $Z$ is the fibre-wise direct sum of $N_M X$ with $\restr{(N_Z M)}{X}$.
	Because $N_Z M$ is a trivial bundle, the normal bundle of $X$ in $Z$ is just $N_M X\times \R$.
	The tubular neighbourhood of $X$ in $Z$ is then an embedding $\ttilde{f}:N_M X\times \HH^1\to Z$ that extends an embedding of a tubular neighbourhood $(X,f:N_M X\to M)$ over a collar of $M$.
\end{defn}
