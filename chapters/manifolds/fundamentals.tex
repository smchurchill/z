...
%\begin{prop}
%	Let $(W,\mathcal{A})$ be an $n$--manifold such that the boundary $M=\pd W$ is an $(n-1)$--manifold.
%	Suppose $M$ has a decomposition $M=\cup_i H_i$ into submanifolds with faces such that $H_i\cap H_j$ is nonempty if and only if $H_i\cap H_j$ is a face of each.
%	The $H_i$ are the \emph{decomposing blocks} of $M$.
%	
%	Then there exists a smooth structure $\mathcal{B}$ on $W$ such that $(W,\mathcal B)$ is a manifold with faces and each face of $(W,\mathcal B)$ is a decomposing block of $M$.	
%\end{prop}
%
%\begin{proof}
%	We obtain $\mathcal{B}$ from $\mathcal{A}$ by restricting the charts of $A$ near the corners of the $H_i$.
%	More specifically, we obtain $\mathcal B$ by removing each chart $(\varphi,U)$ of $\mathcal{A}$ about a given $j$--corner $p$ of $H_i$ if $U$ cannot be restricted to $U'$ in such a way that $\varphi(p)$ is a $(j+1)$--corner in $\varphi(U')$ inside of an appropriate $\HH^k\times\R^{n-k}$, and replacing $(\varphi,U)$ with $(\varphi,U')$ otherwise.
%\end{proof}
