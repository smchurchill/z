...
\begin{prop}[Theorem 3.3 in~\cite{JSP}]
	...
	The boundary of $X\cup_F X'$ is the union of $\pd X\setminus \iota(F)$, $\pd X\setminus \iota'(F)$, and $\pd F$.
	The $j$--boundary of $X\cup_F X'$ consists of $(\pd_j X\setminus i(F))\cup(\pd_j X'\setminus i'(F))\cup\pd_j F$ for each $j$.
\end{prop}
