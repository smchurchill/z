...
\begin{proof}
	...
	We first decompose $X$ into the triad $X=(X;G,M)$ where
	\begin{enumerate}
		\item $G=\iota(F)$,
		\item $G\cup M=\pd X$, and
		\item $G\cap M=\pd G = \pd M =\iota(\pd F)$.
	\end{enumerate}
	We do the same, respectively, to $X'=(X';G',M')$.
	
	Let $h:G\times\HH\to X$ and $h':G'\times\HH \to X'$ be collars of $G$ and $G'$ in $X$ and $X'$.
	Also take
	\[
		\begin{array}{lrcl}
			g_G:&
		...
		\end{array}
	\]
	to be collars of $\pd G$ and $\pd G'$ in their respective codomains, ensuring that 
	\[
		\iota\inv\comp g_G = (\iota')\inv\comp g_G'.
	\]
