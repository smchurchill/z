...
If we hope to glue manifolds together and produce another manifold, that manifold needs an atlas.
This section and the next provide the tools that allow us to construct new atlases from old.
...
Second, the definition is applicable to manifolds with faces, and the tangent spaces obtained are still vector spaces of an appropriate dimension.

\begin{defn}[Derivations on $\R^n$]
	Let $x$ be a point in $\R^n$ and let the space of smooth functions $\R^n\to\R$ be $C^\infty(\R^n)$.
...
We define standard terminology for smooth maps between manifolds.
This helps us describe submanifolds and provides some of the groundwork for defining handles in the next section.

\begin{defn}[Embedding]
	Let $f:X\to M$ be a smooth map between manifolds.
	The \emph{rank} of $f$ at the point $p\in X$ is the rank of $f_*(p)$, the pushforward evaluated at $p$, as a linear map.
...
\begin{defn}[Homotopy]
	...
	\[
		...
		\end{array}
	\]
	and $H_t$ a continuous function $X\to M$ for all $t$, $H_0(x)=f(x)$, and $H_1(x)=g(x)$.
	...
	A deformation retraction is a homotopy equivalence between $M$ and $X$.
\end{defn}
...
\begin{defn}
	Let $M$ be an $n$--manifold with maximal smooth atlas $\mathcal{A}$, let $(\varphi, U)\in\mathcal{A}$, $U$ connected.
	...
	If $(\varphi,U)\in\mathcal{A}$ is a chart with $U$ disconnected, a continuous choice of orientation on each connected component of $U$ is a continuous choice of orientation on $U$.
