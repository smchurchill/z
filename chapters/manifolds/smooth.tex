...
\begin{proof}
	...
	\[
		\begin{array}{rcl}
			\gamma_1'(0)+\gamma_2'(0) & = & (d\varphi)\inv(d\varphi(\gamma_1'(0))+d\varphi(\gamma_2'(0))), \\
			t\gamma_1'(0) & = & (d\varphi)\inv(t\:d\varphi(\gamma_1'(0))).
		\end{array}
	\]
...
\begin{defn}[Embedding]
	...
	If $p$ is a critical point and $d^2f_p = d(df)_p$ is of less than full rank, then $p$ is a \emph{degenerate critical point}.
...
Intersections between $X$ and $Y$ are generically transverse when $\dim X + \dim Y = M$.
A point of $X\cap Y$ that is not a transverse intersection may be perturbed into stable transversality or non-intersection when $\dim X + \dim Y = M$, so we say that intersections of complementary codimension are generically transverse.
...
\begin{defn}
	...
	If the differential $df_p:T_p X\to T_{f(p)} M$ is orientation preserving (resp. orientation reversing) as a map between vector spaces for every $p$ in $X$, then $f$ is \emph{orientation preserving} (resp. \emph{orientation reversing}) as a map.
\end{defn}
